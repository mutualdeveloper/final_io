...
display_action(Action) :-
  ...
  % PHP Binding
iniciamosPhp :- initialize(World,P).%,write("iniciamos..").
mostrarMundoPhp   :-   display_world.
dispararPhp       :-   execute(shoot,P).
caminarPhp        :-   execute(goforward,P).
alzarPhp          :-   execute(grab,P).
girarDerechaPhp   :-   execute(turnright,P).
girarIzquierdaPhp :-   execute(turnleft,P).
subirPhp          :-   execute(climb,P).